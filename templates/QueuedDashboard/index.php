 ...
 * @var \Queue\Model\Entity\QueueJob[] $pendingDetails
 * @var string[] $tasks
 * @var string[] $servers
 * @var array $status
 * @var int $new
...
<nav class="col-md-3 col-12 large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav nav nav-pills flex-column">
        ...
        <li><?php echo $this->Html->link(__d('queue', 'List {0}', __d('queue', 'Queue Jobs')), ['controller' => 'QueueJobs', 'action' => 'index'], ['class' => 'btn margin btn-primary']); ?></li>
    </ul>
</nav>
...
<div class="content col-md-9 col-12 large-9 medium-8 columns">
    ...
    <div class="row">
        <div class="col-md-6 col-12 medium-6 columns">
            ...
            <h2><?php echo __d('queue', 'Queue Jobs'); ?></h2>
            <p>
            ...
            <ol>
                ...
                foreach ($pendingDetails as $pendingJob) {
                    echo '<li>' . $this->Html->link($pendingJob->name, ['controller' => 'QueueJobs', 'action' => 'view', $pendingJob->id]) . ' (ref <code>' . h($pendingJob->reference ?: '-') . '</code>, prio ' . $pendingJob->priority . '):';
                    echo '<ul>';
            ...
            <?php if (Configure::read('Queue.isStatisticEnabled')) { ?>
                <p><?php echo $this->Html->link(__d('queue', 'Detailed Statistics'), ['controller' => 'QueueJobs', 'action' => 'stats']); ?></p>
            <?php } ?>
        </div>
        ...
        <div class="col-md-6 col-12 medium-6 columns">
            ...
            <ul>
                ...
                foreach ($tasks as $task) {
                    ...
                    echo '<li>';
                    echo $this->Form->postLink($task, ['action' => 'addJob', $task], ['confirm' => 'Sure?']);
                    echo '</li>';
                }
                ?>
            </ul>
            ...
            <p><?php echo $this->Html->link(__d('queue', 'Trigger Delayed Test/Demo Job'), ['controller' => 'QueueJobs', 'action' => 'test']); ?></p>
            <?php if (Configure::read('debug')) { ?>
                <p><?php echo $this->Html->link(__d('queue', 'Trigger Execute Job(s)'), ['controller' => 'QueueJobs', 'action' => 'execute']); ?></p>
            <?php } ?>
        </div>
    </div>
